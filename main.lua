...
G.kyubey_shop_tab = G.kyubey_shop_tab or "regular"

G.FUNCS.kyubey_switch_shop_tab = function(e)
    if not e or not e.config or not e.config.kyubey_tab then return end
    G.kyubey_shop_tab = e.config.kyubey_tab
    if G.shop then
        G.shop:remove()
        G.shop = nil
    end
...
local ease_shop_ref = ease_shop
function ease_shop()
    ease_shop_ref()
    
    if not G.shop then return end
    
    local container = G.shop:get_UIE_by_ID('shop_top') or G.shop:get_UIE_by_ID('header') or G.shop
    if container and container.nodes then
        local tab_row = {
            n = G.UIT.R,
            ...
            nodes = {
                UIBox_button({
                    label = {"Regular Shop"},
                    button = "kyubey_switch_shop_tab",
                    minw = 5.5,
                    minh = 0.9,
                    scale = 0.65,
                    colour = G.kyubey_shop_tab == "regular" and G.C.BLUE or G.C.UI.BACKGROUND_INACTIVE,
                    config = {kyubey_tab = "regular"}
                }),
                UIBox_button({
                    label = {"Items"},
                    button = "kyubey_switch_shop_tab",
                    ...
                    minh = 0.9,
                    scale = 0.65,
                    colour = G.kyubey_shop_tab == "items" and G.C.BLUE or G.C.UI.BACKGROUND_INACTIVE,
                    config = {kyubey_tab = "items"}
                })
            }
        }
        table.insert(container.nodes, 1, tab_row)
    end

    if G.kyubey_shop_tab == "items" and G.shop_jokers then
        G.shop_jokers.cards = {}
        -- Add your custom Item cards here later
    end
end